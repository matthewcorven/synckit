...
**MVP scope:** login (handler + trial secretary), trial selection from static list, **left-half** entry form, web-style terms acceptance, database insert, PDF generation into official ASCA PDF template stored as backend asset, email confirmations to handler + secretary.
...
Required on submit:
- Trial selected
- Registration/Tracking # (required by default; configurable)
- Call Name
- Sex
- Owner(s)
...
- At least one grid selection (default)
- Terms accepted
...
**FR-5.1 Template**
- Official ASCA PDF stored as backend static asset or embedded resource.
- Coordinate map per `FormTemplateVersion` stamps fields and grid marks.
...
- Status fields expose failures (PdfStatus/EmailStatus + error code).
...
- FormTemplateVersion supports future ASCA form updates.
