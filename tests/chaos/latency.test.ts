...
describe('Chaos - Latency Injection', () => {
  ...
  it('should converge with 500ms latency', async () => {
    ...
    try {
      ...
      // Extended wait for high latency
      await sleep(8000);
  ...
  it('should converge with 2s latency (extreme)', async () => {
    ...
    try {
      ...
      // Very extended wait for extreme latency
      await sleep(15000);
  ...
  it('should handle variable latency', async () => {
    ...
    try {
      ...
      // Should converge despite variable latency
      const stateA = await clients[0].getDocumentState(docId);
      const stateB = await clients[1].getDocumentState(docId);
  ...
  it('should handle latency spikes', async () => {
    ...
    try {
      ...
      // Wait for convergence despite spikes
      await sleep(12000);
  ...
  it('should handle combined latency and operations', async () => {
    ...
    try {
      ...
      expect(Object.keys(states[0]).length).toBe(15);
    } finally {
      await cleanupChaosClients(clients);
    }
  });
