...
/// <summary>
/// Dataflow storage adapter that uses DataflowDocument instead of Document.
/// 
/// Each document has a TPL Dataflow ActionBlock with MaxDegreeOfParallelism=1,
/// providing ordered sequential processing with built-in backpressure.
/// 
/// Performance characteristics:
/// - Built-in backpressure via BoundedCapacity
/// - Ordered sequential processing per document
/// - Higher base overhead than simple locks
/// - Better burst handling due to buffering
/// </summary>
public class DataflowStorageAdapter : IStorageAdapter, IAsyncDisposable
{
    private readonly ConcurrentDictionary<string, DataflowDocument> _documents = new();
    private readonly ConcurrentDictionary<string, SessionEntry> _sessions = new();
    private readonly ILogger<DataflowStorageAdapter> _logger;

    public DataflowStorageAdapter(ILogger<DataflowStorageAdapter> logger)
    {
    ...
    {
        var doc = _documents.GetOrAdd(id, id2 => new DataflowDocument(id2));
    ...
    {
        var document = _documents.GetOrAdd(documentId, id => new DataflowDocument(id));
    ...
    {
        var document = _documents.GetOrAdd(delta.DocumentId, id => new DataflowDocument(id));
    ...
    public async ValueTask ClearAllAsync(CancellationToken ct = default)
    {
        ...
        _logger.LogInformation("Dataflow storage cleared");
    }
