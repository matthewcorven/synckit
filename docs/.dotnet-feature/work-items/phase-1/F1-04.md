...
- [x] `GET /health` returns 200 with JSON stats (matches TypeScript server)
- [x] Response includes status, version, uptime, stats
- [x] `GET /health/live` returns 200 (liveness probe)
- [x] `GET /health/ready` returns 200 when ready (readiness probe)
- [x] Uptime is accurate (seconds since start)
