...
## Dual-Server Test Harness

The test harness now supports running the same test suite against either the TypeScript or C# server implementation. This is controlled via environment variables:

### Configuration

| Environment Variable | Values | Default | Description |
|---------------------|--------|---------|-------------|
| `TEST_SERVER_TYPE` | `typescript`, `csharp` | `typescript` | Which server implementation to test |
| `SYNCKIT_SERVER_URL` | URL | `ws://localhost:8090/ws` | Server WebSocket endpoint |
| `SYNCKIT_AUTH_REQUIRED` | `true`, `false` | `true` | Whether auth is required |

### Running Tests Against Each Server

```bash
# Run against TypeScript server (default)
cd tests
bun test

# Run against C# server
cd tests
TEST_SERVER_TYPE=csharp bun test

# Run against external server (either implementation)
cd tests
...
The following files were updated to support dual-server testing:

1. **`tests/integration/config.ts`** - Added `TEST_CONFIG.server.type` property
2. **`tests/integration/helpers/test-server.ts`** - Updated to spawn C# server when `type === 'csharp'`
3. **C# Server `Program.cs`** - Added `SYNCKIT_SERVER_URL` support with proper precedence
4. **C# Server `ConnectionManager.cs`** - Added auto-auth when `AuthRequired=false`
5. **C# Server `AuthMessageHandler.cs`** - Added anonymous auth support and `IOptions<SyncKitConfig>` injection
