# V7-04: Run Load Tests

> **Summary:** Run and pass all 73 load tests verifying performance under stress with targets of p99 latency <100ms and throughput >5000 msg/s.

[← Back to Phase 7](README.md) | [Phase Document](../PHASE-7-TESTING.md)

---

**Priority:** P0  
**Estimate:** 4 hours  
**Dependencies:** V7-03

## Description

Run and pass all 73 load tests to verify performance under stress.

## Load Test Scenarios

| Scenario | Connections | Messages/sec | Duration |
|----------|-------------|--------------|----------|
| Basic load | 100 | 1000 | 60s |
| High connections | 1000 | 500 | 60s |
| High throughput | 100 | 10000 | 30s |
| Sustained | 500 | 2000 | 300s |
| Burst | 100 | 50000 | 10s |

## Performance Targets

| Metric | Target | Notes |
|--------|--------|-------|
| p50 latency | <10ms | Median response time |
| p99 latency | <100ms | Tail latency |
| Throughput | >5000 msg/s | Single instance |
| Memory | <500MB | 1000 connections |
| CPU | <80% | Under load |

## Validation Commands

### Against TypeScript Server (baseline)

```bash
cd tests
bun test load/
```

### Against C# Server

```bash
cd tests
TEST_SERVER_TYPE=csharp bun test load/
```

### Against External Server

```bash
cd tests
SYNCKIT_SERVER_URL=ws://localhost:8080/ws bun test load/
```

## Performance Comparison

```bash
# Generate comparison report
bun run load:compare --baseline=typescript --target=dotnet
```

## Acceptance Criteria

- [ ] All 73 load tests pass against TypeScript server (baseline)
- [ ] All 73 load tests pass against C# server
- [ ] p99 latency <100ms for both implementations
- [ ] No memory leaks in either implementation
- [ ] Graceful degradation under overload
- [ ] C# performance within 20% of TypeScript

---

**Status:** ⬜ Not Started
