...
**Notes:** Implemented `AwarenessUpdateMessageHandler` to validate auth and subscriptions, validate state format, store awareness using `IAwarenessStore`, and broadcast `AWARENESS_UPDATE` to other document subscribers using `IConnectionManager.BroadcastToDocumentAsync` (excluding sender). Added unit tests covering subscription checks, broadcasting, and invalid state handling.
