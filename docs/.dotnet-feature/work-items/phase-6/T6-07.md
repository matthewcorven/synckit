...
- [x] Pub/sub messaging tested (covered by C# `RedisPubSubIntegrationTests` for delta and awareness)
- [x] Tests isolated and repeatable (tests create temporary DB instances and temp migration dirs / use schema setup)
- [x] CI/CD uses `server/typescript/docker-compose.yml` (GitHub Actions workflow added at `.github/workflows/integration-tests.yml` and runs Postgres + Redis via that file)
