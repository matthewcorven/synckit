...
- [x] `SchemaValidator` checks for required tables on startup
- [x] Startup fails fast with clear error if schema is missing
- [x] All `IStorageAdapter` methods implemented
- [x] Operations compatible with TypeScript schema (`schema.sql`)
- [x] `documents.state` stores current state (no separate snapshot columns)
- [x] Vector clocks stored in separate table (matches TS)
- [x] Sessions table operations work correctly
- [x] Connection pooling via Npgsql connection string (pooled by driver)
- [x] Transactions for multi-table consistency
- [x] **No schema creation code** - schema managed by T6-03
