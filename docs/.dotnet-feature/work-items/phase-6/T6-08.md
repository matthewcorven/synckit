...
- [x] `RedisAwarenessStore` implements `IAwarenessStore` and is unit-tested. (See `src/SyncKit.Server/Awareness/RedisAwarenessStore.cs` and `SyncKit.Server.Tests/Awareness/RedisAwarenessStoreTests.cs`)
- [x] `SetAsync` enforces clock ordering and returns `false` for stale updates. (Unit test: `SetAsync_RejectsStaleUpdate`)
- [x] Expiration works (entries drop after TTL) and `PruneExpiredAsync` returns expired items reliably in tests. (Integration test: `Expiration_PruneExpired_RemovesEntries`)
- [x] `RemoveAllForConnectionAsync` removes all entries tied to a connection ID. (Unit test: `RemoveAllForConnectionAsync_RemovesMatchingEntries`)
- [x] Integration tests using Testcontainers/Docker validate correctness against a real Redis instance. (See `RedisAwarenessStoreIntegrationTests`)
- [x] DI registration helper `AddRedisAwarenessStorage(IConfigurationSection)` added, and `AddSyncKitStorage` supports `Awareness:Provider=redis`. (See `StorageRegistration.AddRedisAwarenessStorage`)
