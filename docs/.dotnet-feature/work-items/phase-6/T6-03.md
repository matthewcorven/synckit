...
| Approach | Problem |
|----------|---------|
| ❌ Each server manages own migrations | Race conditions, duplicate schemas, version conflicts |
| ❌ TypeScript-only migrations | C# team can't contribute schema changes |
| ✅ **Shared migration tooling** | Single source of truth, runs before any server starts |
...
- [ ] Manual migration command documented
- [ ] Idempotent execution (safe to run multiple times)

## Future Considerations

For versioned migrations (if needed later):
- Add `schema_migrations` table to track applied versions
- Create numbered migration files (`001_`, `002_`, etc.)
- Migration runner applies only unapplied migrations
- Consider using a dedicated migration tool (Flyway, golang-migrate, etc.)
