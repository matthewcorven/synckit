...
|--------------|----------|---------|
| `Sync/IDocumentStore.cs` | `Storage/IStorageAdapter.cs` | Rename interface, add new methods |
...
| File | Changes Needed |
|------|----------------|
| `WebSockets/WebSocketExtensions.cs` | `IDocumentStore` → `IStorageAdapter`, `InMemoryDocumentStore` → `InMemoryStorageAdapter` |
