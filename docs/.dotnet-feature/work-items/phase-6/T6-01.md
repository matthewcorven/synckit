...
5. **Remove obsolete compatibility wrapper** (`InMemoryDocumentStore`)
6. **Update tests** to use new mocks
7. **Remove old interface** after all consumers migrated

## Acceptance Criteria

- [x] Interface renamed to `IStorageAdapter` (matches TS `StorageAdapter`)
- [x] All method names match TypeScript exactly (with `Async` suffix)
- [x] Connection lifecycle methods: `ConnectAsync()`, `DisconnectAsync()`, `IsConnected`
- [x] Document operations: `GetDocumentAsync()`, `SaveDocumentAsync()`, `UpdateDocumentAsync()`, `DeleteDocumentAsync()`, `ListDocumentsAsync()`
- [x] Vector clock operations: `GetVectorClockAsync()`, `UpdateVectorClockAsync()`, `MergeVectorClockAsync()`
- [x] Delta operations: `SaveDeltaAsync()`, `GetDeltasAsync()`, `GetDeltasSinceAsync()` (enhancement)
- [x] Session operations: `SaveSessionAsync()`, `UpdateSessionAsync()`, `DeleteSessionAsync()`, `GetSessionsAsync()`
- [x] Maintenance: `CleanupAsync()`
- [x] DTOs match TypeScript types: `DocumentState`, `DeltaEntry`, `SessionEntry`
- [x] **Existing code migrated** (handlers, tests, DI registration)
...
- Implemented `IStorageAdapter` and DTOs in `server/csharp/src/SyncKit.Server/Storage/` (see `IStorageAdapter.cs`, `Models.cs`, `InMemoryStorageAdapter.cs`).
- Updated message handlers (`Subscribe`, `SyncRequest`, `Delta`) to consume `IStorageAdapter` and use `GetDeltasSinceViaAdapterAsync` where appropriate.
- Added adapter helper `GetDeltasSinceViaAdapterAsync` to support VectorClock-based filtering for existing callers while providing `GetDeltasSinceAsync(documentId, long?)` as a SQL-optimized entrypoint.
- Migrated tests: renamed and updated `InMemoryDocumentStoreTests` → `InMemoryStorageAdapterTests` and updated tests to call the new API. Added new tests for lifecycle, UpdateDocument semantics, session lifecycle and cleanup, and max-clock delta filtering.
- All tests pass locally: `dotnet test ./server/csharp/src/SyncKit.Server.Tests` (688 tests, 0 failures).

- Removed the obsolete compatibility wrapper (`InMemoryDocumentStore`). If you'd like, I can add a Redis/Postgres adapter next.
